...
- An optional typed sandbox `prepare` hook runs before handling and may durably refresh or replace the active resource. It must be retry-safe. Existing-only deliveries never create or reconcile a sandbox, but they do run `prepare` when the exact session already has an active sandbox record.
- `HandlerContext.sandbox.get/getOptional` accepts only the exact `createSandbox` definition configured on that client's environment. `get` requires an active saved resource; `getOptional` returns `undefined` when no active resource exists, including a migrated active record without one. An explicit session end may reconcile uncertain state during the later cleanup phase.
