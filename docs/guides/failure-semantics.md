...
Stored delivery errors, JSON state, and default logs are plaintext and are not redacted automatically. The operational HTTP API omits payloads, metadata, session state, notes, and delivery errors, but this does not redact the underlying store or project logs. Sanitize provider errors before throwing or logging them when they may contain credentials, request bodies, source content, or other sensitive values.
