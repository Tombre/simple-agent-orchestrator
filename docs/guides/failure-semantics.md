...
For ordinary startup, HTTP route setup and binding happen after recovery and environment mounting but before pollers and workers start. A hook or listener failure performs complete startup rollback without allowing polling or worker side effects. A built-in webhook success is returned only after dispatch persistence, while delivery processing remains asynchronous. Shutdown stops accepting requests first, waits for accepted webhook, operational, custom request, and dispatch work, then unmounts environments and releases store ownership. HTTP-close failures are aggregated with other cleanup failures; unresolved listener cleanup remains retryable by a later `stop()`.
...
Stored event content, session state, notes, delivery errors, cursor values, JSON state, and default/project logs are plaintext and are not redacted automatically. The built-in operational HTTP API omits payloads, metadata, session state, notes, and delivery errors, but project middleware and custom handlers control their own access and logging. Sanitize provider errors before throwing or logging them when they may contain credentials, request bodies, source content, or other sensitive values.
