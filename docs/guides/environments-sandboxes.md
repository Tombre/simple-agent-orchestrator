...
Sandbox creation and cleanup are serialized only inside one runtime process. Creation can repeat if external work succeeds before its marker is persisted; cleanup can repeat after an uncertain failure. Cleanup can also succeed before final delivery persistence fails, requiring creation of a new active resource on retry. External hooks must reconcile by stable resource identity across create, cleanup, and recreate, rather than blindly replaying an identifier for a resource that may now be closed. Administratively ending a session with `sessions end` records the end state but does not run sandbox cleanup. State pruning preserves ended sessions whose durable sandbox marker is still true so it does not erase the package's resource record.
